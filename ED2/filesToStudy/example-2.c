...
typedef struct {
    ...
    int max_val;
    char magic[3];
    int removed; // 0 for active, 1 for removed
} IndexRecord;
...
void importImage(char* pgm_path) {
    ...
    // Read header, being careful with comments and whitespace
    fscanf(pgm_fp, "%2s", magic);
    int ch;
    while ((ch = fgetc(pgm_fp)) != EOF && isspace(ch));
    if (ch == '#') {
    ...
    record.removed = 0; // Set flag to active
...
void exportImage(char* image_name, char* output_path, int negative) {
    ...
    while (fread(&record, sizeof(IndexRecord), 1, index_fp) == 1) {
        // Only consider a match if the name is correct AND it's not marked as removed
...
void listImages() {
    ...
    while (fread(&record, sizeof(IndexRecord), 1, index_fp) == 1) {
        // Only list images that are not marked as removed
        if (record.removed == 0) {
...
void removeImage(char* image_name) {
    ...
    IndexRecord record;
    int found = 0;
    long record_pos = 0; // To store the position of the record to be updated

    // Find the record
    while (fread(&record, sizeof(IndexRecord), 1, index_fp) == 1) {
        if (strcmp(record.name, image_name) == 0 && record.removed == 0) {
            found = 1;
            // Go back to the beginning of the record we just read
    ...
    if (found) {
        record.removed = 1; // Mark the record as removed
        fseek(index_fp, record_pos, SEEK_SET); // Seek to its position
        fwrite(&record, sizeof(IndexRecord), 1, index_fp); // Overwrite it
...
int main(int argc, char* argv[]) {
    if (argc < 2) {
        ...
        fprintf(stderr, "  Para remover:  %s remove <nome_imagem_no_banco>\n", argv[0]);
        return 1;
    }

    if (strcmp(argv[1], "import") == 0) {
        if (argc != 3) {
            fprintf(stderr, "Uso incorreto para 'import'. Tente: %s import <caminho_imagem.pgm>\n", argv[0]);
            return 1;
        }
        importImage(argv[2]);
        ...
        if (argc != 3) {
            fprintf(stderr, "Uso incorreto para 'remove'. Tente: %s remove <nome_imagem_no_banco>\n", argv[0]);
            return 1;
        }
        removeImage(argv[2]);
    } else {
